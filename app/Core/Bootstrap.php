...
class Bootstrap
{
    ...
    private function routeApiWithRouter(): void
    {
        ...
        // Participations
        $router->add('POST', '/api/participations/request', [TrajetController::class, 'requestParticipation'], [function () {
            (new \App\Middleware\AuthMiddleware())->authenticate();
        }]);
        $router->add('POST', '/api/participations/validate', [TrajetController::class, 'validateParticipation'], [function () {
            (new \App\Middleware\AuthMiddleware())->authenticate();
        }]);
        $router->add('POST', '/api/participations/confirm', [TrajetController::class, 'confirmParticipation'], [function () {
            (new \App\Middleware\AuthMiddleware())->authenticate();
        }]);
